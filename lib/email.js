...
export async function sendVerificationChangeEmail(emailUser, code) {
    try {
        ...
        await email.sendMail({
            from: `"SplitQ ✌️" <${process.env.OUTLOOK_EMAIL}>`, // sender address
            to: emailUser, // list of receivers
            subject: "Cambio de correo 📧", // Subject line
            html: html2(`${process.env.DOMAIN}/change_email/${code}`)
        });
    } catch (error) {
        console.log(error)
        return {error: "No se ha podido enviar el correo"}
    ...
    <table class="container" cellpadding="20" cellspacing="0">
        <tr>
            <td>
                ...
                <p>Hola</p>
                <p>Vemos que quieres cambiar tu correo. Para completar el cambio, haz clic en el siguiente botón para verificar tu dirección de correo electrónico:</p>
                <a href="${link}" class="button">Verificar cuenta</a>
                <p>Si no puedes hacer clic en el botón, puedes copiar y pegar el siguiente enlace en tu navegador web:</p>
                <p><a class="link" href="${link}">${link}</a></p>
            </td>
